                ...
                <ul class="top-menu text-right list-inline">
                    <li class="dropdown cart-nav dropdown-slide">
                        ...
                        <div class="dropdown-menu cart-dropdown">
                                ...
                                @foreach (auth()->user()->carts  as $cart)
                                    ...
                                    <div class="media">
                                        <a class="pull-left" href="#!">
                                            <img class="media-object" src="{{ asset('uploads/' . $cart->image1) }}"
                                                alt="image" />
                                        </a>
                                        <div class="media-body">
                                            ...
                                            <h5><strong>${{ number_format($cart->quantity * $cart->price) }}</strong></h5>
                                        </div>
                                        <form action="{{ route('site.delete_cart', $cart->id) }}" method="POST">
                                            @csrf
                                            @method('delete')
                                            <button style=" background: transparent; border: 0;" class="remove"><i class="tf-ion-close"></i></button>
                                        </form>
                                    </div><!-- / Cart Item -->
                                @endforeach
                            @endauth
                            @php
                                $cart_total = 0;
                                foreach (auth()->user()->carts  as $info) {
                                    $cart_total += $info->quantity * $info->price;
                                }
                            @endphp
